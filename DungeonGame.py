...
# 	def calculateMinimumHP(self, dungeon):
# 		
# 		self.recrusiveDungeon(dungeon)
# 		
# 		self.getStep(dungeon)
# 		
# 		return self.getNeedHP(dungeon)
# 	
# 	def getStep(self, dungeon):
# 		if(len(self.dict) <= 0):
# 			return
...
# 			downStep = -10000
# 			if(y < len(dungeon)-1):
# 				downStep = self.dict[down] if self.dict.has_key(down) else sum(dungeon[len(dungeon)-1][x-1::])
# 
# 			
# 			rightStep = -10000
# 			if(x<len(dungeon[0])-1):
# 				rightStep = self.dict[right] if self.dict.has_key(right) else sum(item[len(dungeon[0])-1] for item in dungeon)
# 
# 			
# 			if(downStep > rightStep):
# 				self.step.append("down")
# 				y = y+1
# 			else:
# 				self.step.append("left")
# 				x = x+1
# 				
# 			if(x >= len(dungeon[0])-1) and (y >= len(dungeon)-1):
# 				break;
# 			
# 	def recrusiveDungeon(self, dungeon, right=0, top=0):
# 		if len(dungeon) == 0:
# 			return 0
# 		elif len(dungeon) == 1:
# 			s = sum(dungeon[0])
# 			self.dict[str(right)+str(top)] = s
# 			return s
# 		elif len(dungeon[0]) == 1:
# 			s = sum(x[0] for x in dungeon)
# 			self.dict[str(right)+str(top)] = s
# 			return s
# 		else:
# 			downDungeon = dungeon[1::]
# 			downHP = self.dict[str(right) + str(top +1)] \
# 				if self.dict.has_key(str(right) + str(top + 1)) else self.recrusiveDungeon(downDungeon, right, top+1)
# 			
# 			rightDungeon = [x[1::] for x in dungeon]
# 			rightHP = self.dict[str(right + 1) + str(top)] \
# 				if self.dict.has_key(str(right + 1) + str(top)) else self.recrusiveDungeon(rightDungeon, right+1, top)
# 			
# 			nextHP = self.getGoodHP(downHP, rightHP)
# 			
# 			self.dict[str(right)+str(top)] = nextHP + dungeon[0][0]
# 
# 			return nextHP + dungeon[0][0]
# 		
# 	def getGoodHP(self, num1, num2):
...
# 				return num1
# 	
# 	def getNeedHP(self, dungeon):
# 		x = 0
# 		y = 0
# 		point = 0
# 		
# 		if(dungeon[0][0] > 0):
# 			self.curHP = self.curHP + dungeon[0][0]
# 		elif (self.curHP - abs(dungeon[0][0]) > 0):
# 			self.curHP = self.curHP -abs(point)
# 		else:
# 			self.needHP = self.needHP + abs(self.curHP - abs(dungeon[0][0])) + 1
# 			self.curHP = 1
# 		
# 		for s in self.step:
# 
...
# 				point  = dungeon[y][x]
# 				if(point <= 0):
# 					if(self.curHP - abs(point) <= 0):
# 						self.needHP = self.needHP + abs(self.curHP - abs(point)) + 1
# 						self.curHP = 1
# 					else:
# 						self.curHP = self.curHP - abs(point)
# 				else:
# 					self.curHP = self.curHP + point
# 			else:
# 				
# 				y = y + 1
# 				point = dungeon[y][x]
