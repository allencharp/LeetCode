...
class Solution:
	...
	# @return a integer
	def calculateMinimumHP(self, dungeon, left=0, top=0):
		if len(dungeon) == 0:
			return 0
		elif len(dungeon) == 1:
			s = sum(dungeon[0])
			if((left == 0 and top == 0) is not True):
				return s
			else:
				if s <= 0:
					return abs(s) + 1
			...
			downHP = self.dict[str(left) + str(top +1)] \
				if self.dict.has_key(str(left) + str(top + 1)) else self.calculateMinimumHP(downDungeon, left, top+1)
			
			rightDungeon = [x[1::] for x in dungeon]
			rightHP = self.dict[str(left + 1) + str(top)] \
				if self.dict.has_key(str(left + 1) + str(top)) else self.calculateMinimumHP(rightDungeon, left+1, top)
			
			nextHP = self.getMinHP(downHP, rightHP)
			self.dict[str(left)+str(top)] = nextHP + dungeon[0][0]
			return dungeon[0][0] + nextHP

	def getMinHP(self, num1, num2):
